...
internal fun ChromeDPConnection.newSession(sessionId: SessionID) = ChromeDPSession(this, sessionId)

internal fun ChromeDPSession.newSession(sessionId: SessionID) = ChromeDPSession(connection, sessionId)
...
class ChromeDPSession internal constructor(
    ...
    internal suspend fun <I> request(methodName: String, requestParams: I?): InboundFrame =
        connection.request(methodName, requestParams, sessionId)
    ...
    fun <E> events(eventName: String, deserializer: DeserializationStrategy<E>): Flow<E> =
        connection.sessionEvents(sessionId)
            .filter { it.matchesMethod(eventName) }
